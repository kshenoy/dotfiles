...
* bashrc
:PROPERTIES:
:header-args+: :tangle "generated/bash/bashrc"
:END:
...
if [[ -x /usr/bin/dircolors ]]; then
    _dircolors=${XDG_CONFIG_HOME:-$HOME/.config}/dotfiles/generated/dircolors.rc
...
:PROPERTIES:
:header-args+: :tangle (if (executable-find "rg") "generated/bash/bashrc" "no")
:END:
These will be tangled only if =rg= exists
#+begin_src bash
export RIPGREP_CONFIG_PATH=${XDG_CONFIG_HOME:-$HOME/.config}/dotfiles/generated/ripgreprc
alias g='command rg'
#+end_src

#+begin_src conf :tangle (if (executable-find "rg") "generated/ripgreprc" "no")
# This is a tangled file. Do not make any changes here. All changes should preferably be made in the original Org file.
...
#+begin_src bash :tangle (if (string-match "buntu" (system-name)) "generated/bash/bashrc" "no")
...
*** WSL
#+begin_src bash :tangle (if (string-match "windows" (system-name)) "generated/bash/bashrc" "no")
...
* LS_COLORS/dircolors
:PROPERTIES:
:header-args+: :tangle generated/dircolors.rc
:END:
