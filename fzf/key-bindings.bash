...
__fzf_p4_walist__() {                                                                                             # {{{1
  local dir=$(wa_list | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS" $(__fzfcmd) +m)
  printf 'cd %q' "$dir"
}


unset -f __fzf_p4_opened__
__fzf_p4_opened__() {
  cmd='p4 opened | sed -r -e "s/#.*$//" -e "s:^//depot/[^/]*/(trunk|branches/[^/]*):$STEM:"'
...
if [[ -o vi ]]; then
  ...
  # Alt+W: cd into a perforce workarea
