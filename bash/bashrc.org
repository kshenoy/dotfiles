...
#+begin_src bash :tangle (when (string-match "buntu" (system-name)) "bashrc_literate")
...
:PROPERTIES:
:header-args+: :tangle (when (executable-find "rg") "bashrc_literate")
:END:
...
*** home (*buntu)
:PROPERTIES:
:header-args: :tangle (when (string-match "buntu" (system-name)) "bashrc_literate")
:END:
