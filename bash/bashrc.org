...
pushd() {
    ...
    dir_pos=($(dirs -l -v | tail -n+2 | grep "$PWD$" | sed 's/^\s*//' | cut -d ' ' -f1 | paste -s))
    for pos in ${dir_pos[@]}; do
        command popd -n +$pos 2> /dev/null
    done
}
#+end_src
...
cd() {
    ...
    case "$1" in
        ...
        ...)
            local _dir=$PWD;
            $(FZF_ALT_C_COMMAND='_dir=$PWD; while [[ -n "$_dir" ]]; do _dir="${_dir%/[^/]*}"; echo ${_dir:-/}; done' __fzf_cd__)
            return
            ;;
...
# Not adding to dirstack to avoid cluttering it
alias ..='command cd ..'
#+end_src
